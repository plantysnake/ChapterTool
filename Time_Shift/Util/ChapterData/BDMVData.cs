...
namespace ChapterTool.Util.ChapterData
{
    public class BDMVData
    {
        ...
        public static List<ChapterInfo> GetChapter(string location)
        {
            ...
            ProcessStartInfo processStartInfo = new ProcessStartInfo("C:\\Tool\\MeGUI\\tools\\eac3to\\eac3to.exe", location)
            {
                CreateNoWindow = true,
            ...
            foreach (ChapterInfo current in list)
            {
                ...
                process.WaitForExit();
                if (!text.Contains("Chapters")) continue;
